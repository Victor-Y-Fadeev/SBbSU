...
public final class FirstPartTasks {
    ...
    public static List<Album> sortedFavorites(Stream<Album> albums) {
        return albums
                ...
                .sorted((firstAlbum, secondAlbum) -> firstAlbum.getName().compareTo(secondAlbum.getName()))
                .collect(Collectors.toList());
    }
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums
                .map(album -> new Pair<>(album, album
                                .getTracks()
                                .stream()
                                .mapToInt(track -> track.getRating())
                                .max()
                                .orElse(0)))
                .min((firstAlbum, secondAlbum) -> firstAlbum.getValue().compareTo(secondAlbum.getValue()))
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums
                .map(album -> new Pair<>(album, album
                        .getTracks()
                        .stream()
                        .mapToDouble(t -> t.getRating())
                        .average()
                        .orElse(0)))
                .sorted((firstAlbum, secondAlbum) -> secondAlbum.getValue().compareTo(firstAlbum.getValue()))
                .map(album -> album.getKey())
                .collect(Collectors.toList());
    }
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream
                .reduce(1, (firstInt, secondInt) -> firstInt * secondInt % modulo);
    }
